...
namespace micro_inventario.Datos
{
    public class SQLiteCon
    {
        ...
        public Task<List<Producto>> BuscarProducto(string NombreProducto)//Recupera por nombres
        {


            return db.Table<Producto>().Where(i => i.Nombre == NombreProducto).ToListAsync();
        ...
        public Task<double> Sumatoria()
        {
            return db.ExecuteScalarAsync<double>("SELECT SUM(Total) FROM Compras");
        }
